...
const sector = (label: string, categories: string[], enterprises: string[]): TopicSeed => branch(label, [
  branch("General Categories", categories),
  branch("Notable Companies and Enterprises", enterprises)
]);

const period = (label: string, sectors: TopicSeed[], largest: string[]): TopicSeed => branch(label, [
  ...sectors,
  branch("Largest Companies and Enterprises of the Period", largest)
]);

export function buildCompaniesTopic(): TopicSeed {
  return branch("Companies", [
