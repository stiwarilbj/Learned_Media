(function () {
  ...
  function searchCandidates(videos, plan, topic, channelId, limit, excluded, expanded) {
    const skip = {}; (excluded || []).forEach(function (id) { skip[id] = true; }); const groups = (plan.conceptGroups || []).filter(function (group) { return group && group.terms && group.terms.length; }); const terms = (plan.terms || []).concat(plan.include || []).concat(expanded ? (plan.alternatives || []) : []).filter(Boolean).slice(0, 36); const selectedChannel = plan.channelId || channelId; const wantedTopics = (plan.topics || []).map(norm); const excludes = (plan.exclude || []).map(norm).filter(Boolean); const results = [];
