(function () {
  ...
  function filter(videos, query, topic, channelId) {
    const term = norm(query); return (videos || []).filter(function (video) { if (!isAllowed(video)) return false; if (channelId && video.channelId !== channelId) return false; if (topic && topic !== "All" && !(video.topics || []).includes(topic)) return false; if (!term) return true; const fields = [{ name: "title", value: video.title }, { name: "description", value: video.description }, { name: "tags", value: (video.tags || []).join(" ") }, { name: "topics", value: (video.topics || []).join(" ") }]; const words = term.split(" ").filter(function (word) { return word.length > 1; }); return words.every(function (word) { return fields.some(function (field) { const value = norm(field.value); return value.indexOf(word) >= 0 || value.split(" ").some(function (candidate) { return word.length >= 5 && candidate.length >= 5 && distance(word, candidate) <= (word.length >= 8 ? 2 : 1); }); }); }); });
  }
  function cleanDescription(value) { return String(value || "").replace(/(?:subscribe|like and subscribe|follow us|social media|patreon|sponsor(?:ed)? by|use code|affiliate|merch(?:andise)?|join the discord|business inquiries|check out my|support the channel)[^.!?]*(?:[.!?]|$)/gi, " ").replace(/https?:\/\/\S+/gi, " ").replace(/\s+/g, " ").trim().slice(0, 1800); }
  function distance(left, right) {
    if (left === right) return 0;
    const rows = Array.from({ length: right.length + 1 }, function (_, index) { return index; });
    for (let row = 1; row <= left.length; row += 1) { let diagonal = rows[0]; rows[0] = row; for (let column = 1; column <= right.length; column += 1) { const above = rows[column]; rows[column] = Math.min(rows[column] + 1, rows[column - 1] + 1, diagonal + (left[row - 1] === right[column - 1] ? 0 : 1)); diagonal = above; } }
    return rows[right.length];
  }
  function termMatches(term, fields) {
    const wanted = norm(term); if (!wanted) return { matched: false, score: 0, fields: [], support: [] };
    const wantedWords = wanted.split(" ").filter(function (word) { return word.length > 1; }); let score = 0; const matchedFields = []; const support = [];
    fields.forEach(function (field) { const value = norm(field.value); const words = value.split(" ").filter(Boolean); const phrase = value.indexOf(wanted) >= 0; const matched = wantedWords.filter(function (word) { return words.some(function (candidate) { return candidate === word || word.length >= 5 && candidate.length >= 5 && distance(word, candidate) <= (word.length >= 8 ? 2 : 1); }); }); if (!phrase && (!matched.length || matched.length < Math.max(1, Math.ceil(wantedWords.length * .5)))) return; const multiplier = field.name === "title" ? 8 : field.name === "tags" ? 4 : field.name === "topics" ? 3 : field.name === "description" ? 2 : 1; score += multiplier * (phrase ? 2 : matched.length / Math.max(wantedWords.length, 1)); matchedFields.push(field.name); if (field.name === "title" || field.name === "description") support.push(field.value.slice(0, 220)); });
    return { matched: score > 0, score: score, fields: Array.from(new Set(matchedFields)), support: Array.from(new Set(support)).slice(0, 3) };
  }
  function broadTokenScore(term, fields) {
    const wanted = norm(term).split(" ").filter(function (word) { return word.length > 2; });
    if (!wanted.length) return 0;
    const searchableWords = norm(fields.map(function (field) { return field.value; }).join(" ")).split(" ").filter(Boolean);
    const matched = wanted.filter(function (word) { return searchableWords.some(function (candidate) { return candidate === word || word.length >= 5 && candidate.length >= 5 && distance(word, candidate) <= (word.length >= 8 ? 2 : 1); }); });
    return matched.length / wanted.length;
  }
  function searchCandidates(videos, plan, topic, channelId, limit, excluded, expanded) {
    const skip = {}; (excluded || []).forEach(function (id) { skip[id] = true; }); const groups = (plan.conceptGroups || []).filter(function (group) { return group && group.terms && group.terms.length; }); const rawTerms = (plan.terms || []).concat(plan.include || [], plan.alternatives || [], expanded ? (plan.alternatives || []) : []).filter(Boolean); const seenTerms = {}; const terms = rawTerms.filter(function (term) { const key = norm(term); if (!key || seenTerms[key]) return false; seenTerms[key] = true; return true; }).slice(0, 48); const selectedChannel = plan.channelId || channelId; const wantedTopics = (plan.topics || []).map(norm); const excludes = (plan.exclude || []).map(norm).filter(Boolean); const results = []; const semanticFallback = [];
    (videos || []).forEach(function (video) {
      if (skip[video.id] || !isAllowed(video) || selectedChannel && video.channelId !== selectedChannel || topic && topic !== "All" && !(video.topics || []).includes(topic) || wantedTopics.length && !(video.topics || []).some(function (item) { return wantedTopics.indexOf(norm(item)) >= 0; })) return;
      if ((plan.dateIntent || "upload") !== "event" && plan.minDate && video.publishedAt < plan.minDate || (plan.dateIntent || "upload") !== "event" && plan.maxDate && video.publishedAt > plan.maxDate || plan.minDurationSeconds !== undefined && video.durationSeconds < Number(plan.minDurationSeconds) || plan.maxDurationSeconds !== undefined && video.durationSeconds > Number(plan.maxDurationSeconds)) return;
      const fields = [{ name: "title", value: video.title }, { name: "description", value: cleanDescription(video.description) }, { name: "tags", value: (video.tags || []).join(" ") }, { name: "topics", value: (video.topics || []).join(" ") }]; const haystack = norm(fields.map(function (field) { return field.value; }).join(" ")); if (excludes.some(function (term) { return haystack.indexOf(term) >= 0; })) return;
      const matches = terms.map(function (term) { return termMatches(term, fields); }).filter(function (match) { return match.matched; }); const groupMatches = groups.map(function (group) { return group.terms.map(function (term) { return termMatches(term, fields); }).filter(function (match) { return match.matched; }).sort(function (a, b) { return b.score - a.score; })[0]; }); if (groups.some(function (group, index) { return group.required !== false && !groupMatches[index]; })) return; const allMatches = matches.concat(groupMatches.filter(Boolean)); const softScore = terms.length ? Math.max.apply(Math, terms.map(function (term) { return broadTokenScore(term, fields); }).concat([0])) : 0; if (!allMatches.length && !wantedTopics.length && softScore < .25) { semanticFallback.push({ video: video, score: softScore, matchedFields: ["semantic-fallback"], supportingText: [] }); return; } results.push({ video: video, score: allMatches.reduce(function (sum, match) { return sum + match.score; }, 0) + (allMatches.length ? softScore * .35 : softScore), matchedFields: Array.from(new Set(allMatches.reduce(function (all, match) { return all.concat(match.fields); }, []).concat(allMatches.length ? [] : ["semantic-fallback"]))), supportingText: Array.from(new Set(allMatches.reduce(function (all, match) { return all.concat(match.support); }, []))).slice(0, 3) });
    });
    const maxResults = Math.max(1, Math.min(80, limit || 80)); return results.length || wantedTopics.length ? results.sort(function (a, b) { return b.score - a.score; }).slice(0, maxResults) : semanticFallback.sort(function (a, b) { return b.score - a.score; }).slice(0, maxResults);
  }
  ...
  window.LEARNED_MEDIA_YOUTUBE = { CHANNELS: CHANNELS, TOPICS: TOPICS, sync: sync, filter: filter, searchCandidates: searchCandidates, shuffle: shuffle };
})();
